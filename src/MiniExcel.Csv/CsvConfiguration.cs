...
public class CsvConfiguration : MiniExcelBaseConfiguration
{
    ...
    {
        get => field ??= stream => new StreamReader(stream, DefaultEncoding, true, BufferSize, leaveOpen: true);
        set;
    }

    public Func<Stream, StreamWriter> StreamWriterFunc
    {
        get => field ??= stream => new StreamWriter(stream, DefaultEncoding, BufferSize);
        set;
    }
}
