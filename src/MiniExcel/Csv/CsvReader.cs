...
internal partial class CsvReader(Stream stream, IMiniExcelConfiguration? configuration) : IExcelReader
{
    private readonly Stream _stream = stream;
    private readonly CsvConfiguration _config = configuration is null ? CsvConfiguration.DefaultConfiguration : (CsvConfiguration)configuration;
    ...
    public async IAsyncEnumerable<IDictionary<string, object?>> QueryAsync(bool useHeaderRow, string? sheetName, string startCell, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        for (var rowIndex = 1; (row = await reader.ReadLineAsync(
            ...
            cancellationToken
#endif
            ).ConfigureAwait(false)) is not null; rowIndex++)
        {
            string finalRow = row;
            ...
            if (read.Length < headRows.Count)
            {
                ...
                throw new ExcelColumnNotFoundException(columnIndex: null, headRows[colIndex], [], rowIndex, headers, rowValues, $"Csv read error: Column {colIndex} not found in Row {rowIndex}");
            }
