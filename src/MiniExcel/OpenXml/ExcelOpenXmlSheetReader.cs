...
internal partial class ExcelOpenXmlSheetReader : IExcelReader
{
    ...
    private ExcelOpenXmlSheetReader(Stream stream, IMiniExcelConfiguration? configuration)
    {
    ...
    [CreateSyncVersion]
    public static async Task<ExcelOpenXmlSheetReader> CreateAsync(Stream stream, IMiniExcelConfiguration? configuration, CancellationToken cancellationToken = default)
    {
        var reader = new ExcelOpenXmlSheetReader(stream, configuration);
    ...
    {
        return QueryImplAsync<T>(QueryRangeAsync(false, sheetName, startCell, endCell, cancellationToken), startCell, hasHeader, _config, cancellationToken);
    }
    ...
    [CreateSyncVersion]
    public static async IAsyncEnumerable<T> QueryImplAsync<T>(IAsyncEnumerable<IDictionary<string, object?>> values, string startCell, bool hasHeader, MiniExcelConfiguration configuration, [EnumeratorCancellation] CancellationToken cancellationToken = default) where T : class, new()
    {
        cancellationToken.ThrowIfCancellationRequested();
