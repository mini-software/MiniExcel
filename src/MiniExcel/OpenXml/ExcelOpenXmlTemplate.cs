                    ...
                    foreach (XmlElement c in row.SelectNodes($"x:c", _ns))
                    {
                        ...
                        foreach (var item in matchs)
                        {
                            ...
                            if (keys[0].StartsWith("$")) //e.g:"$rowindex"
                                continue;
                            if (!inputMaps.ContainsKey(keys[0]))
                                throw new System.Collections.Generic.KeyNotFoundException($"Please check {keys[0]} parameter it's not exist.");
                        ...
                        foreach (XmlElement c in cs)
                        {
                            ...
                            foreach (var item in matchs)
                            {
                                ...
                                if (cellValue is IEnumerable && !(cellValue is string))
                                {
                                    ...
                                    if (rowCotainIEnumerable == false)
                                    {
                                        ...
                                        foreach (var element in cellValue as IEnumerable)
                                        {
                                            if (element != null)
                                            {
                                                ...
                                                props = ienumerableGenricType.GetProperties().ToDictionary(s => s.Name, s => s);
                                                break;
                                            }
                                        }
                                    ...
                                    var type = nullableUnderlyingType??prop.PropertyType; //avoid nullable 
                                                                                              // 
