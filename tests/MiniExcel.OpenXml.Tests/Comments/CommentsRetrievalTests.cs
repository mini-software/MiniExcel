...
public class CommentsRetrievalTests
{
    private readonly OpenXmlImporter _excelImporter =  MiniExcel.Importers.GetOpenXmlImporter();

    [Fact]
    public void SheetWithNotesAndCommentsWithRepliesTest()
    {
        var commentSet = _excelImporter.RetrieveComments(PathHelper.GetFile("xlsx/TestCommentsAndNotes.xlsx"), "sheet1");
        ...
        Assert.Equal(new Guid("00000000-0006-0000-0000-000001000000"), commentSet.Notes[0].Id);
        Assert.Equal("D6", commentSet.Notes[0].ReferenceCell);
        Assert.Empty(commentSet.Notes[0].Author ?? "");
        Assert.Equal("this is a simple note", commentSet.Notes[0].Text);

        Assert.Equal(new Guid("4e01653b-66e0-48be-9390-2bddb28a7255"), commentSet.Notes[1].Id);
        Assert.Equal("G10", commentSet.Notes[1].ReferenceCell);
        Assert.Equal("local user", commentSet.Notes[1].Author);
        Assert.Equal("local user:\nthis is a note from someone else", commentSet.Notes[1].Text);
    }
        
    [Fact]
    public void SheetWithNotesAndCommentsWithoutRepliesTest()
    {
        var commentSet = _excelImporter.RetrieveComments(PathHelper.GetFile("xlsx/TestCommentsAndNotes.xlsx"), "sheet2");
    ...
    [Fact]
    public void SheetWithoutNotesNorCommentsTest()
    {
        var commentSet = _excelImporter.RetrieveComments(PathHelper.GetFile("xlsx/TestCommentsAndNotes.xlsx"), "sheet3");
    ...
    [Fact]
    public void SheetWithResolvedThreadedCommentsTest()
    {
        var commentSet = _excelImporter.RetrieveComments(PathHelper.GetFile("xlsx/TestCommentsAndNotes.xlsx"), "sheet4");
        var comment = commentSet.Comments[0];
