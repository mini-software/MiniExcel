...
public class MiniExcelOpenXmlMultipleSheetAsyncTests
{
    ...
    public async Task SpecifySheetNameQueryTest()
    {
        ...
        var rows4 = await _excelImporter.QueryAsync(path, sheetName: "Sheet3").Cast<IDictionary<string, object>>().ToListAsync();
        Assert.Equal(5, rows4.Count);
        Assert.Equal(3d, rows4[0]["A"]);
        Assert.Equal(3d, rows4[0]["B"]);

        var rows5 = await _excelImporter.QueryAsync(path, sheetName: "Sheet2").Cast<IDictionary<string, object>>().ToListAsync();
        Assert.Equal(12, rows5.Count);
        Assert.Equal(1d, rows5[0]["A"]);
        Assert.Equal(1d, rows5[0]["B"]);
        
        var rows6 = await _excelImporter.QueryAsync(path, sheetName: "Sheet1").Cast<IDictionary<string, object>>().ToListAsync();
        Assert.Equal(12, rows6.Count);
        Assert.Equal(2d, rows6[0]["A"]);
        Assert.Equal(2d, rows6[0]["B"]);

        var rows7 = await _excelImporter.QueryAsync(path, sheetName: "Sheet1").Cast<IDictionary<string, object>>().ToListAsync();
    ...
    public async Task MultiSheetsQueryBasicTest()
    {
        ...
        _ = await _excelImporter.QueryAsync(stream, sheetName: "Sheet1", leaveOpen: true).ToListAsync();
        _ = await _excelImporter.QueryAsync(stream, sheetName: "Sheet2", leaveOpen: true).ToListAsync();
    ...
    public async Task MultiSheetsQueryTest()
    {
        ...
        await using var stream = File.OpenRead(path);
        var sheetNames2 = await  _excelImporter.GetSheetNamesAsync(stream, leaveOpen: true);

        Assert.Equal(["Sheet1", "Sheet2", "Sheet3"], sheetNames2);
        foreach (var sheetName in sheetNames2)
        {
            _ = await _excelImporter.QueryAsync(stream, sheetName: sheetName, leaveOpen: true).ToListAsync();
        }
    }
}
