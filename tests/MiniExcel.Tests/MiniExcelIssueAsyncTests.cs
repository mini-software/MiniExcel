...
public class MiniExcelIssueAsyncTests(ITestOutputHelper output)
{
    ...
    public async Task Issue227()
    {
...

        {
            ...
            {
                var q =  _excelImporter.QueryAsync<MiniExcelOpenXmlTests.UserAccount>(path).ToBlockingEnumerable();
                var rows = q.ToList();
            ...
            {
                await using var stream = File.OpenRead(path);
                var q =  _excelImporter.QueryAsync<MiniExcelOpenXmlTests.UserAccount>(stream).ToBlockingEnumerable();
                var rows = q.ToList();
                Assert.Equal(100, rows.Count);
...
#pragma warning restore CS0618
    ...
    public async Task Issue215()
    {
        ...
        var q =  _excelImporter.QueryAsync(stream, true).ToBlockingEnumerable().Cast<IDictionary<string, object>>();
        var rows = q.ToList();
    ...
    public async Task Issue142_Query()
    {
        ...
        {
            var q =  _excelImporter.QueryAsync<Issue142VO>(path).ToBlockingEnumerable();
            var rows = q.ToList();
    ...
    public async Task Issue149()
    {
...

        {
            ...
            var q =  _excelImporter.QueryAsync<Issue149VO>(path).ToBlockingEnumerable();
