...
namespace MiniExcelLibs.Tests
{
    public partial class MiniExcelOpenXmlTests
    {
        ...
        [Fact]
        public void CustomPropertyActionTest()
        {
            var detailList = new List<dynamic>()
            {
                new { Id = 1, Name="github", Att04 = "1-Att04", Att03 = "1-Att03", Att01 = "1-Att01", Att02 = "1-Att02"},
            };
...

            {
                ...
                Action<List<MiniExcelLibs.Utils.ExcelCustomPropertyInfo>> customPropertyAction = _ =>
                {
                    //1.需要移除列
                    List<String> removeColumns = new List<string>();
                    removeColumns.Add("Att02");
                    removeColumns.Add("Att04");

                    foreach (var item in removeColumns)
                    {
                        _.RemoveAll(c => c.ExcelColumnName == item);
                    }
                    //2.自定义列名
                    ...
                    {
                        if (keyValues.ContainsKey(item.ExcelColumnName))
                        {
                            item.ExcelColumnName = keyValues[item.ExcelColumnName];
                        }
                    }
                };

                MiniExcel.SaveAs(path, sheets, configuration: new OpenXmlConfiguration() { CustomPropertyAction = customPropertyAction });

                var rows = MiniExcel.Query(path, false).ToList();
                Assert.Equal("自定义列3", rows[0].C);
                Assert.Equal("自定义列1", rows[0].D);
            }
...

            {
                ...
                MiniExcel.SaveAs(path, sheets, configuration: new OpenXmlConfiguration() { CustomPropertyAction = null });

                var rows = MiniExcel.Query(path, false).ToList();
                Assert.Equal("Att04", rows[0].C);
                Assert.Equal("Att03", rows[0].D);
                Assert.Equal("Att01", rows[0].E);
                Assert.Equal("Att02", rows[0].F);
            }
        }
    }
}